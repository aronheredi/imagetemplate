...
export default function CommonStyle({ propUpdate, selectedObject }: CommonStyleProps) {
    ...
    return (<div className="flex flex-col gap-3">
        ...
        <EditField fields={
            [{
                label: 'Opacity', input: <Input type='number' min={0} max={1} step={0.01} value={selectedObject.opacity || 1} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { propUpdate('opacity', Number.parseFloat(e.target.value)); }} />
            }]
        } />
    </div>)
}