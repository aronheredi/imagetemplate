...
export const QueryBuilderPopup = ({ canvas }: QueryBuilderPopupProps) => {
    ...
    useEffect(() => {
        ...
        const transformedValues = Object.entries(values).map(([objName, props]) => {
            const propertyObject: Record<string, string | number | boolean | undefined> = { name: objName };
            props.forEach(prop => {
                    ...
                    <form onSubmit={onSubmit} className="space-y-4 flex flex-col gap-2 overflow-auto p-4">
                        ...
                        <div className='flex flex-col gap-4 overflow-scroll max-h-[40vh]'>
                            {canvas.getObjects().map((obj) => (
                                <div key={obj.name} className='border border-slate-200 bg-slate-50 rounded-lg p-4 w-full flex flex-col gap-2'>
                                    ...
                                    {values[obj.name!]?.map((prop) => (
                                        <TextField.Root onChange={(e) => setPropertyValue(obj.name!, prop.key, e.target.value)} key={prop.key} placeholder='Prop name' radius='large' size='3' className=' gap-2 border-slate-300 py-4 px-3 shadow-sm focus:border-primary focus:ring-1 focus:ring-primary'>
                                    ...
                                    <DropdownMenu.Root >
                                        ...
                                        <DropdownMenu.Portal >
                                            ...
                                            >
                                                {fabricQuerySchema[obj.type].filter((prop) => !(values[obj.name!]?.some((p) => p.key === prop.key))).map((prop) => {

                                                    return (

                                                        <DropdownMenu.Item key={prop.key} onSelect={() => addProperty(obj.name!, { ...prop, value: obj.get(prop.key) })} className="px-3 py-2 rounded-md hover:bg-slate-100 text-sm cursor-pointer">
                                                            {prop.key}
                                                        </DropdownMenu.Item>
                                                    );
                                                })
